...
static devfs_type_t dev_types_list[] = {
    {
        .name = "tty",
        .dev_type = DEV_TTY,
        .file_type = FILE_TTY,
...
int devfs_open(struct _fs_t * fs,const char* path,file_t * file){
    for(int i = 0 ; i< (sizeof(dev_types_list)/sizeof(dev_types_list[0]));i++){
        devfs_type_t * type = dev_types_list + i;
        int type_name_len = kernel_strlen(type->name);
        //前缀相等
        if(!kernel_strncmp(path,type->name,type_name_len)){
            int minor;
            
            //传入的地址太短或取次设备号失败    path_to_num返回1代表有错误
            if(kernel_strlen(path) < type_name_len || (path_to_num(path+type_name_len,&minor)<0)){
                log_printf("path length error");
                break;
            }
            
            int dev_id = dev_open(type->dev_type,minor,(void*)0);
            if(dev_id < 0){
                log_printf("dev open error");
                break;
            }
